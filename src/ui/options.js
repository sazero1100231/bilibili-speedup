...
function fillForm(config) {
  ...
  for (const host of config.candidateHosts) {
    ...
    option.value = host;
    option.textContent =
      host === config.selectedHost ? `${host}（測速首選）` : host;
...
async function load() {
  ...
  if (!response?.ok) {
    throw new Error(response?.error ?? "無法讀取設定");
  }
...
document.querySelector("#save").addEventListener("click", async () => {
  controls.status.textContent = "儲存中…";
  try {
    const settings = readSettings();
    await chrome.storage.local.set({ settings });
    runtimeConfig.settings = settings;
    controls.status.textContent = "已儲存。建議重載已開啟的 Bilibili 頁面。";
  } catch (error) {
    controls.status.textContent = `儲存失敗：${error.message}`;
  }
});

document.querySelector("#clearProbe").addEventListener("click", async () => {
  controls.status.textContent = "清除中…";
  const response = await chrome.runtime.sendMessage({
  ...
  if (response?.ok) {
    runtimeConfig = response.config;
    fillForm(runtimeConfig);
    controls.status.textContent = "測速快取已清除，下一支影片會重新測速。";
  } else {
    controls.status.textContent = `清除失敗：${response?.error ?? "未知錯誤"}`;
  }
});
...
load().catch((error) => {
  controls.status.textContent = `載入失敗：${error.message}`;
});
